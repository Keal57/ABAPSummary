...
CLASS lhc_travel IMPLEMENTATION.
  ...
  METHOD showtestmessage.
    DATA message TYPE REF TO zku_cm_travel.

    message = NEW zku_cm_travel( severity  = if_abap_behv_message=>severity-success
                              textid    = zku_cm_travel=>test_message
                              user_name = sy-uname ).
  ...
  METHOD validateagency.
    ...
    " Read Travels
    READ ENTITY IN LOCAL MODE ZKU_R_Travel
         FIELDS ( AgencyId )
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF exists = abap_false.
        message = NEW zku_cm_travel( textid    = zku_cm_travel=>no_agency_found
  ...
  METHOD validatecustomer.
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF exists = abap_false.
        message = NEW zku_cm_travel( textid      = zku_cm_travel=>no_customer_found
  ...
  METHOD validatedates.
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-EndDate < travel-BeginDate.
        message = NEW zku_cm_travel( textid = zku_cm_travel=>invalid_dates ).
        APPEND VALUE #( %tky = travel-%tky
                        %msg = message ) TO reported-travel.
        APPEND VALUE #( %tky = travel-%tky ) TO failed-travel.
      ENDIF.
  ...
  ENDMETHOD.
    ...
    MODIFY ENTITY IN LOCAL MODE ZKU_R_Travel
...
METHOD canceltravel.
    ...
    LOOP AT travels REFERENCE INTO DATA(travel).
      ...
      IF travel->Status = 'X'.
        message = NEW zku_cm_travel( textid = zku_cm_travel=>travel_already_cancelled
      ...
      travel->Status = 'X'.
      message = NEW zku_cm_travel( severity = if_abap_behv_message=>severity-success
                                textid   = zku_cm_travel=>travel_cancelled_successfully
                                description   = travel->Description ).
      APPEND VALUE #( %tky     = travel->%tky
                      %element = VALUE #( Status = if_abap_behv=>mk-on )
                      %msg     = message ) TO reported-travel.
